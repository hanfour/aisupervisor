...
func (c *Config) Validate() error {
	...
	validCLITools := map[string]bool{"claude": true, "aider": true, "ais-agent": true, "": true}
	for _, wt := range c.WorkerTiers {
		...
		if !validCLITools[wt.CLITool] {
			return fmt.Errorf("invalid cli_tool %q for tier %q (must be claude, aider, or ais-agent)", wt.CLITool, wt.Tier)
		}
		if wt.MaxWorkers < 0 {
